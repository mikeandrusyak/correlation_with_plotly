...
Creates separate scatter traces for each species (robust for RStudio and HTML export) so the dropdown can toggle visibility deterministically.

```{r}
fig <- plot_ly() %>%
  add_markers(
    data = iris %>% filter(Species == "setosa"),          # Setosa points
    x = ~Sepal.Length,
    y = ~Petal.Length,
    type = 'scatter',
    mode = 'markers',
  ...
  add_markers(
    data = iris %>% filter(Species == "versicolor"),      # Versicolor points
    x = ~Sepal.Length,
    y = ~Petal.Length,
    type = 'scatter',
    mode = 'markers',
  ...
  add_markers(
    data = iris %>% filter(Species == "virginica"),       # Virginica points
    x = ~Sepal.Length,
    y = ~Petal.Length,
    type = 'scatter',
    mode = 'markers',
...
Includes a dropdown menu that toggles trace visibility (3 scatter + 3 lines) instead of using filter transforms. This ensures that "All" shows all groups on initial load.
      ...
      list(
        buttons = list(
          list(
            method = "update",
            args = list(
              list(visible = c(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE))  # All traces visible
            ),
            label = "All"
          ),
          list(
            method = "update",
            args = list(
              list(visible = c(TRUE, FALSE, FALSE, TRUE, FALSE, FALSE)) # Setosa + its line
            ),
            label = "Iris-setosa"
          ),
          list(
            method = "update",
            args = list(
              list(visible = c(FALSE, TRUE, FALSE, FALSE, TRUE, FALSE)) # Versicolor + its line
            ),
          ...
          list(
            method = "update",
            args = list(
              list(visible = c(FALSE, FALSE, TRUE, FALSE, FALSE, TRUE)) # Virginica + its line
            ),
